...
<div class="stateDetails">
    <div v-for="(item,i) of list" :key="i" style="display:block;">
        ...
        toStore(){
           ...
           this.axios.get(url).then(res=>{
               if(res.data.code==-1){
                   ...
                   if(list.tosid==1){
                       this.order.state="待付款";
                   }else if(list.tosid==2){
                       this.order.state="待发货";
                   }else if(list.tosid==3){
                       this.order.state="待收货";
                   }else{
                       this.order.state="待评价";
                   }
                   this.list=list;
